...
import { getFloor, strings } from '@/config';
...
/**
 * Requirement #7: while escorting, the kiosk shows the smiling face with the
 * "시설 안내중" caption. The subtitle reflects same-floor vs the cross-floor
 * handoff (#6). On arrival it briefly celebrates, then the controller returns
 * the kiosk to patrol (#10).
 */
export function GuidingScreen() {
  ...
  const subtitle = arrived ? undefined : describe(session);
...
function describe(session: NavigationSession): string {
  const { plan, progress } = session;

  if (plan.kind === 'cross-floor' && plan.transfer) {
    const toFloor = getFloor(plan.transfer.toFloorId)?.name ?? '';
