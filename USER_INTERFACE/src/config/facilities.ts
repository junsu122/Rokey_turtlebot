import type { Facility } from '@/core/domain';

/**
 * Station facilities, modeled after the real 1F / 2F section drawings.
 * `footprint` is the drawn box (blueprint units, viewBox 160×100), `position` is
 * the marker / ROS goal pose, `variant`/`rotation` are drawing hints. The
 * turtlebot charging bay ("대기") is intentionally omitted from the map.
 */
export const facilities: Facility[] = [
  // ============================ 1층 (F1) ============================
  {
    id: 'f1-exit1',
    name: '1번 출구',
    category: 'exit',
    variant: 'door',
    floorId: 'F1',
    footprint: { x: 9, y: 11, w: 24, h: 12 }, // top-left corner
    position: { x: 26, y: 19 },
    aliases: ['일번출구', '1번', '출구', '출입구', '나가는곳', '밖'],
  },
  {
    id: 'f1-info',
    name: '안내데스크',
    category: 'info',
    floorId: 'F1',
    footprint: { x: 43, y: 11, w: 20, h: 8 }, // top-center
    position: { x: 77, y: 20 },
    aliases: ['안내', '데스크', '인포', '문의', '도움', '고객'],
  },
  {
    id: 'f1-elevator',
    name: '엘리베이터',
    category: 'elevator',
    floorId: 'F1',
    footprint: { x: 72, y: 11, w: 24, h: 22 }, // right of the info desk
    position: { x: 116, y: 25 },
    aliases: ['엘베', '승강기', '리프트', 'ev', '이브이'],
  },
  {
    id: 'f1-restroom',
    name: '화장실',
    category: 'restroom',
    floorId: 'F1',
    footprint: { x: 9, y: 69, w: 20, h: 20 }, // bottom-left corner
    position: { x: 29, y: 69 },
    aliases: ['변소', '토일렛', '용변', '볼일', 'wc', '화장실'],
  },
  {
    id: 'f1-escalator',
    name: '에스컬레이터',
    category: 'escalator',
    floorId: 'F1',
    footprint: { x: 35, y: 77, w: 30, h: 12 }, // bottom-center
    position: { x: 77, y: 78 },
    aliases: ['에스컬', '무빙워크', '올라가는'],
  },
  {
    id: 'f1-exit2',
    name: '2번 출구',
    category: 'exit',
    variant: 'door',
    floorId: 'F1',
    footprint: { x: 127, y: 77, w: 24, h: 12 }, // bottom-right corner
    position: { x: 132, y: 78 },
    aliases: ['이번출구', '2번', '출구', '출입구', '나가는곳', '밖'],
  },

  // ============================ 2층 (F2) ============================
  {
    id: 'f2-pa1',
    name: '탑승구 A-1',
    category: 'platform',
    floorId: 'F2',
    footprint: { x: 9, y: 11, w: 15, h: 7 }, // left column (train to the left)
    position: { x: 39, y: 21 },
    aliases: ['에이원', 'a1', '탑승구', '승강장', '플랫폼', '타는곳'],
  },
  {
    id: 'f2-pa2',
    name: '탑승구 A-2',
    category: 'platform',
    floorId: 'F2',
    footprint: { x: 9, y: 20, w: 15, h: 7 },
    position: { x: 39, y: 37 },
    aliases: ['에이투', 'a2', '탑승구', '승강장', '플랫폼'],
  },
  {
    id: 'f2-pa3',
    name: '탑승구 A-3',
    category: 'platform',
    floorId: 'F2',
    footprint: { x: 9, y: 29, w: 15, h: 7 },
    position: { x: 39, y: 53 },
    aliases: ['에이쓰리', 'a3', '탑승구', '승강장', '플랫폼'],
  },
  {
    id: 'f2-transfer',
    name: '환승출구',
    category: 'transit',
    variant: 'door',
    floorId: 'F2',
    footprint: { x: 138, y: 11, w: 13, h: 10 }, // top-right corner
    position: { x: 135, y: 21 },
    aliases: ['환승', '갈아타', '환승통로', '환승출구'],
  },
  {
    id: 'f2-bench1',
    name: '벤치 1',
    category: 'bench',
    variant: 'a',
    // rotation: 22,
    floorId: 'F2',
    footprint: { x: 45, y: 26, w: 20, h: 10 }, // upper (ahead on y), faces bench 2
    position: { x: 74, y: 35 },
    aliases: ['벤치', '의자', '쉼터', '앉', '소파', '쉬는곳'],
  },
  {
    id: 'f2-bench2',
    name: '벤치 2',
    category: 'bench',
    variant: 'b',
    // rotation: 200,
    floorId: 'F2',
    footprint: { x: 86, y: 33, w: 16, h: 12 }, // lower-right, faces bench 1
    position: { x: 102, y: 51 },
    aliases: ['벤치', '의자', '쉼터', '앉', '소파', '쉬는곳'],
  },
  {
    id: 'f2-gate-normal',
    name: '개찰구',
    category: 'gate',
    variant: 'narrow',
    floorId: 'F2',
    footprint: { x: 34, y: 60, w: 22, h: 12 }, // on the fare line, above the elevator
    position: { x: 77, y: 66 },
    aliases: ['개찰구', '게이트', '일반', '탑승', '타는곳'],
  },
  {
    id: 'f2-gate-accessible',
    name: '개찰구(장애인용)',
    category: 'gate',
    variant: 'wide',
    floorId: 'F2',
    footprint: { x: 86, y: 60, w: 30, h: 12 }, // on the fare line, above the escalator
    position: { x: 131, y: 66 },
    aliases: ['개찰구', '장애인', '휠체어', '우대', '넓은'],
  },
  {
    id: 'f2-elevator',
    name: '엘리베이터',
    category: 'elevator',
    floorId: 'F2',
    footprint: { x: 72, y: 78, w: 22, h: 11 }, // bottom-center
    position: { x: 77, y: 80 },
    aliases: ['엘베', '승강기', '리프트', 'ev', '이브이'],
  },
  {
    id: 'f2-escalator',
    name: '에스컬레이터',
    category: 'escalator',
    floorId: 'F2',
    footprint: { x: 121, y: 78, w: 30, h: 11 }, // bottom-right corner
    position: { x: 133, y: 80 },
    aliases: ['에스컬', '무빙워크', '내려가는'],
  },
];
